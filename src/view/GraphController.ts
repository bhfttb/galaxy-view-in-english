...
clearRoute: () => {
	...
	private bindContextLost(canvas: HTMLElement): void {
		const onLost = (e: Event) => {
			...
			const btn = mask.createEl('button', { cls: 'gx-mask-btn', text: t('context_lost') });
	...
	private playEstablishing(): void {
		...
		this.maskEl.createDiv({ cls: 'gx-mask-text', text: t('building_graph') });
		// 等几帧让首批渲染就绪，再揭幕拉出
		window.setTimeout(() => {
	...
	private initLayout(initialAlpha: number): void {
		...
		try {
			...
			this.layout.init(this.store.data, this.store.positions, params, initialAlpha);
			new Notice(t('worker_unavailable'));
		}
	}
	...
	applyTier(): void {
		...
		if (this.tier.nodeCap !== null && total > this.tier.nodeCap && prev !== this.tier.id) {
			new Notice(t('mobile_tier_nodes', { cap: this.tier.nodeCap, total }));
		}
	}
	...
	private watchdog(now: number): void {
		...
		if (this.hudFrames.length > 0 && this.hudFrames.length < 30 && !this.paused) {
			...
			if (this.lowFpsChecks >= 3) {
				...
				this.applyTier();
				new Notice(t('auto_performance_mode'));
			}
		} else {
			this.lowFpsChecks = 0;
		}
	...
	shuffleColors(): void {
		...
		if (groups.length < 2) {
			new Notice(t('import_colors_first'));
			return;
		}
	...
	private async importColors(notify: boolean): Promise<void> {
		...
		if (!groups || groups.length === 0) {
			if (notify) new Notice(t('no_graph_json_groups'));
			return;
		}
		this.settings.colorGroups = groups;
		this.renderer?.setColorFn(makeNodeColorFn(groups));
...
private updateRoute(): void {
	...
	private updateHud(now: number): void {
		...
		el.setText(
			`${this.hudFrames.length} fps · ${this.renderer?.drawCalls ?? 0} calls · ${c.nodes}n/${c.links}l · ` +
				`${this.layout.isSettled() ? t('settled') : t('layouting')}`,
		);
	}
	...
	private async benchOrbit(scenario: 'S1' | 'S3'): Promise<BenchResult> {
		...
		new Notice(t('waiting_settle', { scenario }));
		...
		new Notice(t('orbit_fps_test', { scenario }));
		...
		new Notice(t('scenario_completed', { scenario, fps: stats.avgFps.toFixed(1), drawCalls: renderer.drawCalls }));
		return result;
	}

	private async benchColdLayout(): Promise<BenchResult> {
		new Notice(t('s2_started'));
		...
		await writeBenchResult(this.app, result);
		new Notice(t('s2_completed', { sec: (settleMs / 1000).toFixed(1), ticks, longest: lt.longestMs.toFixed(0) }));
		return result;
	}
